use std::{io::{Error, ErrorKind}, path::Path, str::FromStr};
use crate::{ensight_gold::{reader::EnsightGoldReader, types::ElementsType, Reader}, utils};
...
impl MeshElementType {
    pub fn read(reader: &mut EnsightGoldReader, ignore_element_id: bool) -> std::io::Result<Self> {
        ...
        if etype != ElementsType::Nfaced {
            ...
            println!("{}", max);
            Ok(Self {n_nodes,n_elements,etype})
